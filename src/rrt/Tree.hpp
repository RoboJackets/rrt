...
public:
   ...
   * is a number in the range [0, 1] that determines what proportion of the time
   ...
   * reached.
   * @details The RRT will continue to run unti we're within @goalMaxDist of the
   * goal state.
   */
  ...
  bool run() {
    ...
    for (int i = 0; i < _maxIterations; i++) {
      ...
      if (newNode &&
          _stateSpace->distance(newNode->state(), _goalState) < _goalMaxDist)
        return true;
    }
   ...
   */
  Node<T> *grow() {
    //  extend towards goal, waypoint, or random state depending on the biases
    //  and a random number
    float r = rand() /
  ...
  virtual Node<T> *extend(const T &target, Node<T> *source = nullptr) {
    ...
    if (_isASCEnabled) {
      ...
      intermediateState =
          _stateSpace->intermediateState(source->state(), target, stepSize());
    }
