...
GridStateSpace::GridStateSpace(float width, float height, int discretizedWidth,
...
Vector2f GridStateSpace::intermediateState(const Vector2f &source,
  ...
  float stepSize = (dist / maxStepSize) *
...
bool GridStateSpace::transitionValid(const Vector2f &from,
                                     const Vector2f &to) const {
  ...
  for (int x = x1; x <= x2; x++) {
    for (int y = y1; y <= y2; y++) {
      if (_obstacleGrid.obstacleAt(x, y)) {
        ...
        Vector2f brCorner(ulCorner.x() + gridSqWidth,
                          ulCorner.y() + gridSqHeight);

        if (delta.x() != 0) {
          /**
           * Find slope and y-intercept of the line passing through @from and
           * @to.
           * y1 = m*x1+b
           * b = y1-m*x1
           */
          ...
          /*
           * First check intersection with the vertical segments of the box.
           * Use y=mx+b for the from-to line and plug in the x value for each
           * wall
           * If the corresponding y-value is within the y-bounds of the vertical
          ...
          if (inRange<float>(from.x(), ulCorner.x(), brCorner.x())) {
            //  order by y-value
            //  note: @lower has a smaller value of y, but will appear higher
            //  visually on the screen due to qt's coordinate layout
            Vector2f lower(from);
            Vector2f higher(to);
            if (higher.y() < lower.y())
              swap<Vector2f>(lower, higher);

            //  check for intersection based on y-values
            if (lower.y() < ulCorner.y() && higher.y() > ulCorner.y())
              return false;
            if (lower.y() < brCorner.y() && higher.y() > brCorner.y())
              return false;
          }
        }
      }
